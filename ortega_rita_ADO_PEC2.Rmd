...
### Representación gráfica (*Smear plots*) de las tres comparaciones {.tabset}
...
abline(h = c(-2,2), col = "blue")
```
###  Volcano plots de las comparaciones entre grupos {.tabset}
...
Para poder hacer la anotación de los resultados, utilizaremos una función diseñada específicamente y la aplicaremos a las tres comparaciones que estamos estudiando:

```{r la cacasion NUSE}
annotatedTopTable <- function(topTab, anotPackage)
{
  topTab <- cbind(Ensembl=rownames(topTab), topTab)
  topTab$Ensembl <- gsub("\\..*", "", topTab$Ensembl, fixed=FALSE)
  myProbes <- topTab$Ensembl
  thePackage <- eval(parse(text = anotPackage))
  geneAnots <- select(thePackage, myProbes, c("SYMBOL", "ENTREZID", "GENENAME"))
  annotatedTopTab<- merge(x=geneAnots, y=topTab, by.x="Ensembl", by.y="Ensembl")
